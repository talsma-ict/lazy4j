...
/// A map that stores its values in a [Lazy] manner.
///
/// This has the advantage that while providing 'standard' [Map] features,
/// unused values do not need to be evaluated.
///
/// The behaviour of this map depends on the delegate map it was [initialized][#LazyValueMap(Supplier)] with.
...
/// The [default][#LazyValueMap()] and [copy][#LazyValueMap(Map)] constructors create a lazy map
/// that behaves like a [LinkedHashMap].
/// If the backing map does _not_ support `null` values, the lazy map _will_ support them,
/// because the values are wrapped in a [Lazy] object, therefore will never be `null` in the backing map.
///
...
public class LazyValueMap<K, V> extends AbstractMap<K, V> {
    ...
    /// Creates a new empty `LazyValueMap`, behaving like a [LinkedHashMap].
    ///
    ...
    /// Creates a new `LazyValueMap` with the same contents as the specified map.
    ///
    /// The new map behaves like a [LinkedHashMap].
    ///
    ...
    /// Creates a new `LazyValueMap` backed by a map created by the specified map factory.
    ///
    ...
    /// ```
    ///
    /// Technically, the mapFactory does not _need_ to create a new map,
    ...
    ///
    /// @param mapFactory The map factory to provide the backing map (required, must provide a non-`null` map).
    ...
    /// @return The lazy value for the specified key, or `null` if the map contains no mapping for the key.
    /// @see #get(Object)
    ...
    ///
    /// The supplied [Supplier] will be evaluated at most once,
    /// the first time the value is required (e.g. via [#get(Object)] or by an operation that
    /// needs the concrete value).
    ///
    /// @param key           the key with which the specified lazy value is to be associated.
    /// @param valueSupplier supplier that provides the value when needed (required, non-`null`).
    /// @return the previous value associated with the key, or `null` if there was no mapping.
    /// @implNote The returned previous mapping, if any, is _not_ eagerly evaluated by this method.
    ...
    /// Lazy variant of [#putIfAbsent(Object, Object)].
    ///
    ...
    /// @param value supplier of the value to associate if absent (required, non-`null`).
    /// @return the existing lazy value if present, or `null` if the association was added.
    public Lazy<V> lazyPutIfAbsent(K key, Supplier<? extends V> value) {
        return delegate.putIfAbsent(key, Lazy.of(value));
    }

    /// Lazy variant of [#compute(Object, BiFunction)].
    ///
    /// Stores a lazy remapping that invokes `remappingFunction` with the key and the evaluated
    /// current value (which may be `null`), when needed.
    ///
    ...
    /// Lazy variant of [#computeIfAbsent(Object, Function)].
    ///
    ...
    /// Lazy variant of [#computeIfPresent(Object, BiFunction)].
    ///
    /// If a non-null mapping exists, stores a lazy remapping that invokes `remappingFunction`
    /// with the key and the evaluated current value when needed.
    ///
    /// @param key               key with which the specified value is associated.
    /// @param remappingFunction the function to compute a value.
    /// @return the new lazy value associated with the specified key, or `null` if none.
    ...
    /// Lazy variant of [#replace(Object, Object)].
    ///
    ...
    /// @param value supplier of the new value (required, non-`null`).
    /// @return the previous lazy value associated with the key, or `null` if there was no mapping.
    ...
    /// Lazy variant of [#merge(Object, Object, BiFunction)].
    ///
    /// If the key is absent, associates it with a lazy value from the given supplier. If present,
    /// associates it with a lazy value that lazily applies the `remappingFunction` to the
    /// existing and supplied values only when needed for the first time.
    ///
    /// @param key               key with which the resulting value is to be associated.
    /// @param value             supplier of the value to use if absent (required, non-`null`).
    /// @param remappingFunction the function to recompute a value if present.
    /// @return the new lazy value associated with the specified key.
    /// @throws NullPointerException if `value` is `null` (via [Lazy#of(Supplier)]).
    ...
    /// Returns a [Collection] view of the lazy values contained in this map.
    ///
    ...
    ///
    /// The set supports mutation operations such as removal and [Map.Entry#setValue(Object)] calls
    /// _if_ the backing map supports them.
    ///
    /// @return A `Set` view of the mapping entries contained in this map.
    @Override
    ...
    ///
    /// @param key the key whose associated value is to be returned
    /// @return The value for the specified key, or `null` if the map contains no mapping for the key.
    /// @see #getLazy(Object)
    @Override
    ...
    ///
    /// This value will **not** be lazy and will be available immediately.
    ///
    /// @param key   the key with which the specified value is to be associated.
    /// @param value the new value to be associated with the specified key.
    /// @return The previous value associated with the specified key, or `null` if there was no mapping for the key.
    /// A null result can also indicate that **the previous lazy value was not yet available**.
    /// @implNote The returned previous mapping, if any, is _not_ eagerly evaluated by this method.
    ...
    ///
    /// The removed value is returned only if it had already been evaluated; otherwise `null` is returned
    /// without forcing evaluation.
    ///
    /// @param key key whose mapping is to be removed from the map.
    /// @return the previously associated value if it was available, or `null` otherwise.
    @Override
    ...
    /// untouched and returned if it was already available; otherwise `null` is returned without
    /// forcing evaluation of the existing lazy value.
    ///
    /// @param key   key with which the specified value is to be associated.
    /// @param value value to associate with the specified key if absent.
    /// @return the previous value if present and already available; otherwise `null`.
    ...
    /// already been evaluated; otherwise `null` is returned without forcing evaluation.
    ///
    /// @param key   key with which the specified value is associated.
    /// @param value value to be associated with the specified key.
    /// @return the previous value if present and available; otherwise `null`.
    ...
    /// Consider using [#lazyComputeIfPresent(Object, BiFunction)] instead, to prevent eager evaluation.
    ///
    /// @param key               key with which the specified value is associated.
    /// @param remappingFunction the function to compute a value.
    ...
    /// Compute a mapping for the specified key and its current mapped value (or `null`
    /// if there is no current mapping).
    ///
    /// If the map contains a lazy value for the specified key,
    /// it will be eagerly evaluated and applied in the remapping function.
    /// Consider using [#lazyCompute(Object, BiFunction)] instead, to prevent eager evaluation.
    ///
    /// @param key               key with which the specified value is associated.
    /// @param remappingFunction the function to compute a value.
    /// @return the new value associated with the specified key, or `null` if none.
    ...
    /// @implNote To comply with the general [Map] contract,
    /// this involves eagerly evaluating **all** lazy values currently contained in the map.
    @Override
    public int hashCode() {
    ...
    /// Therefore, if the map is found to be equal, _all_ its lazy entries will have been eagerly evaluated.
    @Override
    ...
    ///
    /// All values are represented by their [Lazy#toString()] representation.
    /// This prevents unnecessary eager evaluation of values by calling `LazyValueMap.toString()`.
    ///
    ...
    /// EntrySet that exposes `Map.Entry<K,V>` views while delegating to an underlying
    /// `Set<Entry<K, Lazy<V>>>`. Values are evaluated on access only where required.
    ///
    /// @param <K> key type.
    /// @param <V> value type.
    ...
    /// Map.Entry wrapper that exposes an evaluated `V` view over an underlying
    /// `Entry<K, Lazy<V>>`. Setting a value replaces the underlying lazy value with
    ...
    private static final class LazyEntry<K, V> implements Entry<K, V> {
        ...
        /// @implNote This implementation is based on the [Map.Entry#hashCode()] contract.
        ...
        /// The string representation of lazy entries will _not_ eagerly evaluate the underlying lazy value.
    ...
    /// Creates a `Map<K, Lazy<V>>` copy of the given `Map<K, V>`.
    ///
    /// If the source map is already a `LazyValueMap`, its internal lazy values are reused
    /// without triggering evaluation. Otherwise, values are wrapped into eager [Lazy] instances.
    ///
    /// @param toCopy source map to copy from.
    /// @param <K>    key type.
    /// @param <V>    value type.
    /// @return a new [LinkedHashMap] containing lazy values that mirror the source contents.
