...
/// A list that stores its values as [Lazy] references.
///
/// This has the advantage that while providing 'standard' [List] features,
/// unused values do not need to be evaluated.
///
/// The characteristics of this list depend on the delegate list it was initialized with, see [#using(List)].
///
///   - If the delegate list is mutable, the lazy list will be mutable as well.
///   - If the delegate list is thread-safe, the lazy list will be thread-safe as well.
///   - If the lazy list is sorted, its purpose may be defeated because all values have to be evaluated eagerly for comparison.
///
///
/// The [#create()] and [#copyOf(Collection)] methods create a new lazy list with the characteristics of an [ArrayList].
///
/// @param <T> The type of values in the list.
/// @author Sjoerd Talsma
/// @implNote If the backing list does _not_ support `null` values, the lazy list _will_ support them,
/// because all values are wrapped in [Lazy] references, therefore will never be `null` in the backing list.
/// @see Lazy
...
public class LazyList<T> extends AbstractList<T> {
    ...
    ///
    /// The returned list will be backed by an [ArrayList], implementing [RandomAccess].
    ///
    /// To provide the backing list implementation, call [#using(List)].
    ///
    /// @param <T> The type of lazy values in the list.
    /// @return A new, empty `LazyList`, behaving like an [ArrayList].
    ...
    /// Creates a new `LazyList` with the same contents as the specified collection.
    ///
    /// The new list behaves like a [ArrayList].
    ///
    /// @param toCopy The collection to copy the contents from.
    /// @param <T>    The type of lazy values in the list.
    /// @return A new `LazyList` with the same contents as the specified collection.
    /// @see #create()
    /// @see #using(List)
    ...
    /// Creates a new `LazyList` backed by a list to contain the [Lazy] values.
    ///
    /// This offers full control over the backing list implementation.
    /// For instance:
    /// <pre>
    /// `List<T> lazyArrayList = LazyList.using(new ArrayList<>());List<T> lazyLinkedList = LazyList.using(new LinkedList<>());`</pre>
    ///
    /// Technically, the backing list does not _need_ to be a new list,
    /// but please be aware that sharing it list may cause unexpected behaviour,
    /// especially if accessed concurrently.
    ///
    /// @param backingList The backing list (required, must provide a non-`null` list).
    /// @param <T>         The type of lazy values in the list.
    /// @return A new `LazyList` backed by the specified list.
    /// @see #create()
    /// @see #copyOf(Collection)
    ...
    /// Constructor for `LazyList` backed by the specified `delegate` list.
    ///
    ...
    ///   - [#using(List)]
    ///
    /// If you wish to create a subclass, please consider whether the delegate list is a [RandomAccess] list.
    /// If it is, implementing [RandomAccess] or subclassing the [LazyRandomAccessList] may be a good idea.
    ///
    ...
    /// Adds the specified element to the end of this list, without eagerly evaluating it.
    ///
    /// This is another variant of [#addLazy(Supplier)].
    ///
    /// @param lazy The supplier for the new element, evaluated only if necessary.
    /// @see #addLazy(Supplier)
    /// @see #add(Object)
    ...
    /// Otherwise `null` is returned without forcing evaluation.
    ///
    ...
    /// @return The previous value at the specified index, if present and available. Otherwise `null`.
    ...
    /// @return `true` if the values were inserted in the list.
    /// @see #addAllLazy(int, Collection)
    /// @see #addAllLazy(Collection)
    @Override
    ...
    /// @return the removed value if present and available; otherwise `null`.
    @Override
    ...
    ///
    /// The operator will be applied _lazily_ when the result is needed for the first time.
    ...
    /// Sorts this list in accordance with the specified [Comparator].
    ///
    /// Sorting will eagerly evaluate _all_ lazy values currently contained in the list.
    ///
    ...
    /// Removes _all_ elements from the list, regardless of whether they have already been evaluated.
    @Override
    public void clear() {
        delegate.clear();
    }
    ...
    /// This means that _all_ lazy values 'left' of the found element
    /// will have been eagerly evaluated.
    /// If no matching element is found, the method returns `-1`
    /// and _all_ elements will have been eagerly evaluated.
    ///
    /// @param value the element to search for.
    /// @return The index of the first matching element, or `-1` if no match was found.
    /// @implNote Note that getting the index of an element may evaluate more elements
    /// than calling [#contains(Object)] does, because the latter uses a two-pass algorithm
    ...
    /// This means that _all_ lazy values to the 'right' of the found element
    /// will have been eagerly evaluated.
    /// If no matching element is found, the method returns `-1`
    /// and _all_ elements will have been eagerly evaluated.
    ///
    /// @param value the element to search for.
    /// @return The index of the last matching element, or `-1` if no match was found.
    ...
    /// If the method returns `false`, _all_ lazy values in the list have been evaluated.
    /// @see #streamAvailable()
    /// @see #streamLazy()
    ...
    /// @return `true` if the requested element was removed from this collection.
    /// @implNote This implementation uses a two-pass algorithm where the first pass checks available values only.
    /// This avoids unnecessary eager evaluation of lazy values when the searched value is already evaluated.
    /// If the method returns `false`, _all_ lazy values have been evaluated.
    @Override
    ...
    ///
    /// New [Lazy] values [added][#addLazy(Supplier)] to the sublist will be shared by both lists,
    /// still evaluating them only once, when needed.
    ///
    /// @param fromIndex low endpoint (inclusive) of the subList
    /// @param toIndex   high endpoint (exclusive) of the subList
    /// @return A new [LazyList] containing the elements between the specified fromIndex (inclusive) and toIndex (exclusive).
    @Override
    ...
    /// @implNote To comply with the general [List] contract,
    /// this involves eagerly evaluating lazy values until one is encountered that is not equal to the item from the other list.
    /// Therefore, if the list is found to be equal, _all_ its lazy entries will have been eagerly evaluated.
    @Override
    ...
    ///
    /// Iterated values are evaluated eagerly. See [#streamAvailable()] or [#streamLazy()]
    /// for streams that do not eagerly evaluate lazy values.
    ///
    /// This is the equivalent of calling `streamLazy().map(Lazy::get)`.
    ///
    ...
    /// `streamLazy().filter(Lazy::isAvailable).map(Lazy::get)`.
    ///
    ...
    ///
    /// Iterated values are evaluated eagerly. See [#parallelStreamAvailable()] or [#parallelStreamLazy()]
    /// for streams that do not eagerly evaluate lazy values.
    ///
    /// This is the equivalent of calling `parallelStreamLazy().map(Lazy::get)`.
    ///
    ...
    ///
    /// This is the equivalent of calling
    /// `parallelStreamLazy().filter(Lazy::isAvailable).map(Lazy::get)`.
    ///
    ...
    /// Creates a `List<Lazy<T>>` copy of the given `Collection<T>`.
    ///
    /// If the source collection is already a `LazyList`, its internal lazy values are reused
    ...
    /// @return a new [ArrayList] containing lazy values that mirror the source contents.
    ...
    /// [LazyList] that implements [RandomAccess].
    ///
    ...
    /// Lazy Spliterator implementation for [LazyList].
    ///
