...
public final class LazyListIterator<T> implements LazyIterator<T>, ListIterator<T> {
    ...
    ///
    /// To be called by [LazyList#listIterator()].
    ///
    ...
    ///
    /// @return `true` if there are more elements in the iteration, otherwise `false`.
    @Override
    ...
    /// Returns `true` if there are any previous elements in the iteration.
    ///
    /// @return `true` if there are previous elements in the iteration, otherwise `false`.
    @Override
    ...
    /// Returns the index of the element that would be returned by a later call to next or nextLazy.
    ///
    ...
    /// Returns the index of the element that would be returned by a later call to previous or previousLazy.
    ///
    ...
    /// by [#next()]/[#nextLazy()], [#previous()]/[#previousLazy()]
    ...
    /// returned by [#next()]/[#nextLazy()], if any, and after the element that would be returned by
    /// [#previous()]/[#previousLazy()], if any.
    ///
    /// The lazy element will only be evaluated when it is actually needed for the first time by the list.
    ///
    /// @param lazy Lazy supplier for the new element.
